...
def mbm_dict_to_dataframe(
    mbm_dict: Dict, additional_metadata: Optional[Dict] = None
) -> pd.DataFrame:
    """
    ...
    for bead_id, bead_data in mbm_dict.items():
        ...
        df_points = pd.DataFrame(
            {"gri": points["gri"], "tim": points["tim"], "str": points["str"]}
        )
...
def align_datasets_using_beads(
    ...
    df_ref = mbm_dict_to_dataframe(
        reference_mbm_dict, additional_metadata={"type": "reference"}
    )
    df_mov = mbm_dict_to_dataframe(
        moving_mbm_dict, additional_metadata={"type": "moving"}
    )
    ...
    if bead_correspondence is None:
        ...
        bead_correspondence = {
            mov: ref
            for mov, ref in bead_correspondence.items()
    ...
    if len(pts_ref) < 3:
        print(
            f"Warning: Only {len(pts_ref)} bead pair(s) available. Using translation-only alignment."
        )
    ...
    alignment_mode = (
        "translation-only" if len(pts_ref) < 3 else "rigid (rotation + translation)"
    )
