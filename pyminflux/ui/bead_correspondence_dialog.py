...
from pyminflux.correct._bead_alignment import (
    RigidTransform,
...
class BeadCorrespondenceDialog(QDialog):
    ...
    def __init__(
        ...
        self.current_beads = (
            reference_beads  # Current dataset (reference for alignment)
        )
    ...
    def _setup_ui(self):
        ...
        instructions.setStyleSheet(
            "QLabel { color: #333; padding: 8px; background-color: #f8f8f8; border-radius: 4px; }"
        )
        ...
        self.show_transformed_checkbox.setEnabled(
            False
        )  # Disabled until alignment is calculated
        self.show_transformed_checkbox.stateChanged.connect(
            self._on_show_transformed_changed
        )
        ...
        auto_match_btn.setToolTip(
            "Automatically match beads that have identical names in both datasets"
        )
        ...
        self.nearest_neighbor_btn.setEnabled(
            False
        )  # Disabled until alignment is calculated
        ...
        self.mean_residual_label.setToolTip(
            "Mean residual error across all matched bead pairs"
        )
        ...
        self.correspondence_table.setSelectionBehavior(
            QTableWidget.SelectionBehavior.SelectRows
        )
        self.correspondence_table.setSelectionMode(
        ...
        self.fluorophore_naming_widget.set_fluorophores(
            all_fluo_ids, self.existing_names
        )
    ...
    def _populate_tables(self):
        ...
        for row, (current_bead_name, pos) in enumerate(
            sorted(self.current_beads.items())
        ):
    ...
    def _plot_beads(self):
        ...
        if self.existing_fluo_ids:
            current_color = (
                Colors()
                ._get_fid_color(min(self.existing_fluo_ids), as_float=False)
                .tolist()
            )
        else:
        ...
        for bead_name, pos in self.current_beads.items():
            ...
            # Add text label
            text = pg.TextItem(
                text=bead_name, color=tuple(current_color), anchor=(0.5, 1.5)
            )
        ...
        for bead_name, pos in self.new_beads.items():
            ...
            # Add text label
            text = pg.TextItem(
                text=bead_name, color=tuple(new_color), anchor=(0.5, -0.5)
            )
            text.setPos(x_val, y_val)
            self.plot_widget.addItem(text)
            self.text_items.append(text)
    ...
    def _update_correspondence_lines(self):
        ...
        for new_name, current_name in self.correspondence.items():
            if new_name in self.new_beads and current_name in self.current_beads:
                ...
                if self.show_transformed and self.current_transform is not None:
                    new_pos_transformed = self.current_transform(
                        new_pos.reshape(1, -1)
                    )[0]
    ...
    def _on_new_point_clicked(self, plot, points):
        ...
        if len(points) > 0:
            ...
            if shift_pressed:
                ...
                if selected_row >= 0:
                    current_bead_name = self.correspondence_table.item(
                        selected_row, 0
                    ).text()
                    ...
                    for (
                        other_current_name,
                        other_combo,
                    ) in self.correspondence_combos.items():
                        if (
                            other_current_name != current_bead_name
                            and other_combo.currentData() == new_bead_name
                        ):
            ...
            for bead_name in self.new_scatter_items.keys():
                self._highlight_bead(
                    bead_name, is_current=False, highlight=(bead_name == new_bead_name)
                )
            ...
            if row >= 0:
                ...
                if current_bead_name in self.correspondence_combos:
                    ...
                    if new_bead_name is not None:
                        self._highlight_bead(
                            new_bead_name, is_current=False, highlight=True
                        )
    ...
    def _assign_nearest_neighbors(self):
        ...
        if self.current_transform is None:
            self.status_label.setText(
                "Error: No transformation calculated. Run 'Calculate Alignment' first."
            )
            self.status_label.setStyleSheet("QLabel { color: red; font-weight: bold; }")
            return
    ...
    def _calculate_alignment(self):
        ...
        for new_bead_name, current_bead_name in self.correspondence.items():
            if (
                new_bead_name in self.new_beads
                and current_bead_name in self.current_beads
            ):
        ...
        if len(pts_ref) < 1:
            self.status_label.setText(
                "Error: Not enough valid bead pairs for alignment."
            )
            self.status_label.setStyleSheet("QLabel { color: red; font-weight: bold; }")
            return
        ...
        try:
            ...
            self.residuals = {}
            for (new_name, current_name), pt_mov, pt_ref in zip(
                bead_pairs, pts_mov, pts_ref
            ):
    ...
    def _update_status(self):
        ...
        self.status_label.setStyleSheet(
            f"QLabel {{ color: {status_color}; font-weight: bold; }}"
        )
    ...
    def _print_transformation_info(self):
        ...
        if isinstance(self.current_transform, TranslationTransform):
            ...
            print(
                f"Translation vector [z, y, x] (nm): {self.current_transform.translation}"
            )
