...
class PyMinFluxMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def setup_actions_conn(self):
        ...
        self.ui.actionSet_Fluorophore_Names.triggered.connect(
            self.open_fluorophore_naming_dialog
        )
    ...
    def _combine_datasets(
        ...
        existing_fluo_ids = sorted(
            [
                fid
                for fid in self.processor.processed_dataframe["fluo"].unique().tolist()
                if fid > 0
            ]
        )
        next_fluo_id = get_next_fluorophore_id(
            self.processor.reader.processed_dataframe
        )
        ...
        print(
            f"Combine completed. Dataset now has {self.processor.num_fluorophores} fluorophore(s)"
        )
        print(f"Fluorophore names: {fluorophore_names}")
        ...
        self.plotter_toolbar.ui.cbColorCodeSelector.setCurrentIndex(
            ColorCode.BY_FLUO.value
        )
