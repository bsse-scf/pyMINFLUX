...
class BaseWorkflow:
    ...
    def __init__(self, processor: Optional[MinFluxProcessor] = None):
    ...
    def set_processor(self, processor: Optional[MinFluxProcessor]) -> None:
        self.processor = processor
        if hasattr(self.panel, "set_processor"):
            self.panel.set_processor(processor)
...
class TrackingWorkflowPanel(QWidget):
    ...
    def set_processor(self, processor: Optional[MinFluxProcessor]) -> None:
        self.processor = processor
        self.enable_controls(processor is not None)

    def enable_controls(self, enabled: bool = False):
        self.pbSaveData.setVisible(enabled)
...
class TrackingWorkflow(BaseWorkflow):
    ...
    It is intentionally processor-backed for now; collaborators can later
