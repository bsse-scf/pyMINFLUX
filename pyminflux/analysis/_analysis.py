...
def render_xyz(
    ...
    * [code]  https://zenodo.org/record/6563100
    ...
    if render_type == "histogram":
        ...
        # Small grid
        g = np.arange(-L, L + 1)
        xk, yk, zk = np.meshgrid(g, g, g, indexing="ij")
        ...
        for i in range(len(ix)):
            ...
            # Add it to the image
            my, mx, mz = np.meshgrid(
                gy, gx, gz, indexing="ij"
            )  # We need to create meshgrid to add the k matrix
            my = Ny - my - 1  # Images have y = 0 at the top
            try:
...
def drift_correction_time_windows_2d(
    ...
    """Estimate drift correction based on auto-correlation.
    ...
    * [code]  https://zenodo.org/record/6563100
    ...
    for j in range(Ns):
        ...
        ti[j] = np.mean(t[idx])
        hj, _, _, _ = render_xy(
            x[idx], y[idx], sxy, sxy, rx, ry, render_type="fixed_gaussian", fwhm=3 * sxy
        )
        h[j] = hj
