...
def test_unmixing_single_fluorophore_into_two():
    ...
    all_existing_fluo_ids = set(
        np.unique(processor.processed_dataframe["fluo"].to_numpy()).astype(int)
    )
    ...
    for i in range(1, len(unique_clusters)):
        while (
            next_id in all_existing_fluo_ids or next_id in new_fluo_id_mapping.values()
        ):
            next_id += 1
    ...
    remapped_fluo_ids = np.array(
        [new_fluo_id_mapping[cid] for cid in cluster_ids], dtype=np.uint8
    )
    ...
    final_fluo_ids = sorted(
        np.unique(processor.processed_dataframe["fluo"].to_numpy()).tolist()
    )
